...
export const LinkedinUrlEnum = pgEnum('urlType', ['profile', 'company', 'post', 'job']);

export const linkedInDataTable = pgTable('linkedin_data', {
    id: uuid('id').primaryKey().defaultRandom(),
    linkedinUrl: text('linkedin_url').notNull(),
    urlType: LinkedinUrlEnum().default(LinkedinUrlEnum.enumValues[0]), // it can be: profile, company, post, job
